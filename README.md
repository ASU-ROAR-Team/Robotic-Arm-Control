...
This repository contains the ROAR arm MoveIt setup and three teleoperation scripts.
...
python3 src/ROAR_pkg/scripts/simple_mover.py

# Wrist-lock teleop (lock pitch + twist joints, then move XYZ)
python3 src/ROAR_pkg/scripts/wrist_lock_teleop.py

# Macro orientation teleop (save/apply orientation macros + XYZ teleop)
python3 src/ROAR_pkg/scripts/macro_orientation_teleop.py
```
...
- Ensure TF frames are available (`world` and `Link_5`) and `joint_states` are being published.
